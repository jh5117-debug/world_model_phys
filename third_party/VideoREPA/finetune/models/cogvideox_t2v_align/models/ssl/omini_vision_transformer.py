...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, npatch_to_keep=None, mask=None, use_checkpoint=False):
    ...
    def interpolate_pos_encoding(
        ...
        if input_shape is None or patches_layout[0] == 1:
            ...
            # VideoREPA implementation:
            assert class_emb.shape[1] == 0
            pos_embed = cls.interpolate_pos_embed_temporal(pos_embed, [160, 240])
        else:
    ...
    @classmethod
    def interpolate_pos_embed_temporal(cls, pos_embed, target_resolution=[240, 240]):    
        # B, L, C -> BT, H, W, C -> BT, C, H, W
        pos_embed = pos_embed.reshape(
            -1, 16 // 2,
            224 // 16, 224 // 16, 768) # -> B, T, H, W, C NOTE hard coded currently
        patch_size = 16
        pos_embed = pos_embed.reshape(-1, 224 // 16, 224 // 16,
                                        768).permute(0, 3, 1, 2)
        ...
        pos_embed = pos_embed.permute(0, 2, 3, 1).reshape(
            -1, 16 // 2,
            target_resolution[0] // patch_size, target_resolution[1] // patch_size, 768)
        
        B, T, H, W, C = pos_embed.shape
        pos_embed = pos_embed.permute(0, 4, 1, 2, 3)  # [B, C, T, H, W]
        pos_embed = torch.nn.functional.interpolate(       
            pos_embed,
            size=(3 * T, H, W),  
            mode='trilinear',
        ...
        pos_embed = pos_embed.permute(0, 2, 3, 4, 1)  # [B, 3T, H, W, C]
        pos_embed = pos_embed.flatten(1, 3)  # [B, 3T*H*W, C]
        return pos_embed
