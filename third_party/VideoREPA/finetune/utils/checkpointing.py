...
def get_intermediate_ckpt_path(checkpointing_limit: int, step: int, output_dir: str, main_process) -> str:
    ...
    if checkpointing_limit is not None:
        ...
        if len(checkpoints) >= checkpointing_limit and main_process:    # when main_process, save the checkpoint
