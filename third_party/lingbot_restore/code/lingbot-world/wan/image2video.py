    ...
    def __init__(
        ...
        Args:
            ...
            control_type (`str`, *optional*):
                Explicit control type. Use ``act`` for action controls and
                ``cam`` for camera controls.
        ...
        if control_type not in {"act", "cam"}:
            raise ValueError(
                "control_type must be explicitly set to 'act' or 'cam' "
                f"(got {control_type!r}) for checkpoint_dir={checkpoint_dir!r}"
            )
