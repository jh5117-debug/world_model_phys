...
IMPORTANT_LOG_REGEX="${IMPORTANT_LOG_REGEX:-(\[GPU RESET\]|\[TRAIN PLAN\]|\[PROGRESS\]|\[MEM PROBE\]|\[SEQ GEOM\]|Gradient checkpointing patched|Applying block-level gradient checkpointing|\[GPU MEM\] (after_|before_)|ERROR physical_consistency|Training aborted|OutOfMemoryError|Traceback)}"
